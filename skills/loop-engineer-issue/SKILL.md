...
6. Re-read this issue and re-count active runs to resolve races optimistically:
   ...
   - Otherwise proceed.
7. Run `git fetch origin` first — every merge in this loop happens on the remote via `gh pr merge`, so the local default branch is always behind; a worktree based on local HEAD builds on stale code. Then:
   - **Fresh issue:** base the branch on the remote tip, not on local HEAD: `git worktree add <worktree_root>/<issue> -b <branch> origin/<default-branch>`. Use fetch + `origin/`, never `git pull`: pull mutates the main checkout (which may be dirty or shared by concurrent runs), fetch only updates remote-tracking refs.
   - **Resuming a `loop:repairing` issue:** reuse the branch and worktree recorded in the run comment. Fetch first, because `loop-review-pr` may have run `gh pr update-branch`, adding remote commits. If the local branch has no unpushed commits, recreate the worktree at `origin/<branch>`; if it has unpushed commits (the previous run died before pushing — they are the only copy), keep them and rebase onto `origin/<branch>` before continuing.

   Do all implementation and verification inside this worktree. Install dependencies there if verification needs them.
