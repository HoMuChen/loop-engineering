...
5. Resume when the next action is clear. Run `git fetch origin` before comparing branch state — merges and `gh pr update-branch` happen on the remote, so local refs are stale; judging divergence or "unpushed work" against an unfetched branch misreads the run.
